...
iterate.lda.dis_method <- function (itNum=10, distance = "manhattan",maximization="greedy"){
  ks=c("manhattan","euclidean");
  kss= c(5,5,5,10,15,25,15,10,5,50,5,10,10,15);
  for(i in 1:length(ks)){
    dis = paste(ks[i],"/",sep="");
    
    iterate.lda(K=5,iT=itNum,kDir=dis,method="lda",distance = ks[i],maximization=maximization);
  }
  
 # iterate.lda(K=5,iT=itNum,kDir=dis,method="lda",distance = ks[,maximization=maximization);
}



iterate.lda.maximization <- function (itNum=10){
 ...
 "manhattan"
  ...
  #for(i in 1:length(ks)){
   # dis = paste(ks[i],"/",sep="");
    ...
    iterate.lda(K,iT=itNum,kDir,method="lda",distance =distances,maximization=maxs);
 # }
...
iterate.lda.dif_k <- function (itNum=10,others=TRUE, distance = "manhattan",maximization="greedy"){
  ks=c(10,15,20,25,30,35,40,45,50);
  results <- vector
  for(i in 1:length(ks)){
                        ...
                        method="lda")
{
  ...
  kss= c(5,5,5,10,15,25,15,10,5,50,5,10,10,15);
  names <- c("20","30","35","36","40","50","60","70","80","90","10","11","12","13");
  #names <- c("80");
  ...
  for(m in 1:length(names)){
    ...
    for(n in 1:iT)
    {
      if(method=="lda"){
        ...
        tcp <- tcp.lda(testDir=testDir2,truthName=truthName2,K=kss[m],distance = distance[m],maximization=maximization[m]);
