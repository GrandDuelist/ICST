...
tcp.lda <- function(   
    ...
    if (rerunLDA==T || is.null(s$lda)){
        ...
        s$lda.dists <- as.matrix(dist(s$lda$theta, method=distance))
    ...
    }else if(maximization=="max"){
      if (verbose==TRUE){
        cat(sprintf("Running the clustering maximization algorithm...\n"))
      }
      s$whichToTry <- .doTopicGreedy(s$lda$topic.membership) 
    ...
    if (verbose==TRUE){
      cat(sprintf("get priority name of files...\n"))
    }
    
    .getPriorityFileName(s)
